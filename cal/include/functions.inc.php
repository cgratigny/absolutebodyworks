...
function theme_cat_legend ($rows = '', $today = false)
{
	global $template_cat_legend, $CONFIG, $lang_general, $ME;
	if(!$rows) $rows = "1";
	...
	while ($row = db_fetch_object($results))
	{
		$params = array(
			...
			'{CAT_LINK}' => 'href="'.$ME.'?mode=cat&cat_id='.$row->cat_id.'"',
			'{COLOR}' => $row->color
		);
...
function get_referer() {
	// returns the URL of the HTTP_REFERER with the querystring portion
	$HTTP_REFERER = getenv("HTTP_REFERER");
	return $HTTP_REFERER;
	//return strip_querystring($HTTP_REFERER);
}
...
// function that returns a category name if it exists, given a cat_id	
	...
	$query = "SELECT cat_name FROM " . $CONFIG['TABLE_CATEGORIES'] . " WHERE enabled = 1 and cat_id = '$cat_id'";
	$results = db_query($query);
	if(!db_num_rows($results)) return false;
	list($cat_name) = db_fetch_array($results);
