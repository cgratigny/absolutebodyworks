...
Description : Calendar rendering file
...
switch($mode) {
	...
	case 'cat' :
		if(!empty($cat_id)) {
			pageheader(sprintf($lang_cat_events_view['section_title'], get_cat_name($cat_id)));
...
";			
      ...
      while (list(,$event_row) = each($events))
      {
        ...
        {
          $link = "href=\"#\" onclick=\"MM_openBrWindow('cal_popup.php?mode=view&id=".$event->id."','Calendar','toolbar=no,";
          $link .= "location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=".$CONFIG['popup_event_width'].",height=".$CONFIG['popup_event_height']."')\"";
        }
...
function print_weekly_view($date = '')	{
  ...
  if ($CONFIG['weekly_view'] || has_priv('has_admin_access'))
  {
    ...
    {
      echo "<a href=calendar.php?mode=week&date=".date("Y-m-d", mktime(0,0,0,$fdm,$fdd-7,$fdy)).">";
...
function print_monthly_view($date = '')	{
  ...
  if ($CONFIG['monthly_view'] || has_priv('has_admin_access'))
  {
    ...
    for ($i=1;$i<=$nr;$i++)
    {
      ...
      $event_stack[$i]['week_number'] = (int) get_week_number($i + 1, $month, $year);
...
function print_cat_content($cat_id) {
  ...
  if ($CONFIG['cats_view'] || has_priv('has_admin_access')) //  enabled or not ?
  {
    ...
    } else 
    {
      ...
      if (!$total_events) { 
				theme_redirect_dialog(sprintf($lang_cat_events_view['section_title'],$cat_name), $lang_cat_events_view['no_events'], $lang_general['back'], $ME . "?mode=cats");
      } else 
      {
        while ($row = db_fetch_object($result))
        {
				...
				$stat_string = sprintf($lang_cat_events_view['stats_string'], $total_events);
				theme_cat_events_list($event_rows, $cat_name, $stat_string);
          ...
          // Move uploaded picture from the temporary folder
	        ...
	        @chmod($CONFIG['UPLOAD_DIR'].$picture, octdec($CONFIG['picture_chmod'])); 
		        ...
		        $template_vars = array(
						...
						echo strtr($lang_system['event_notification_body'], $template_vars);
