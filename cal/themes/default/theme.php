...
Description : Theme construction file
...
<!-- END header_row -->
<!-- BEGIN today_row -->
				<td bgcolor=$todayclr width='5' height='5' style='border: 1px solid #FFFFFF'>
...
function theme_cat_events_list(&$results, $cat_name, $stats_string) {
    global $template_cat_events_list, $lang_cat_events_view, $today, $lang_date_format;

		$header_row = template_extract_block($template_cat_events_list, 'info_row');
		...
		starttable('100%', sprintf($lang_cat_events_view['section_title'],$cat_name), 2, '', $today_date);
...
function theme_view_event(&$event, $is_popup = false) {
    ...
    global $template_event_view, $lang_event_view, $lang_general, $lang_date_format, $CONFIG, $ME, $REFERER, $THEME_DIR;
		...
		starttable('100%', sprintf($lang_event_view['display_event'],$event->title), 2, '', $print_link.$edit_link.$delete_link );
...
function theme_monthly_view($date, &$results, &$info_data)
{
    ...
    for ($i=1-$firstday;$i<=count($results);$i+=7)
    {
				...
				if ($CONFIG['popup_event_mode'])
        {
          ...
          $event_list .= "status=no,menubar=no,scrollbars=yes,resizable=no,width=".$CONFIG['popup_event_width'].",height=".$CONFIG['popup_event_height']."')\">";
        }
		...
		theme_cat_legend ($CONFIG['cal_view_show_week']?8:7, 1);
    endtable();
